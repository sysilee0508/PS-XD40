...
{
	OSG_Display_Freeze();		// Freeze 표시 
	
	OSG_Display_AUTO();    		// Auto 표시

	OSG_Display_Video_Loss();	//Video Loss 표시
}


//------------------------------------------------------------------------------------------
// 현재 LIVE의 시간(RTC에서 읽어온 BCD데이터)을 buf에 위치에 맞게 넣어 놓는다.
//------------------------------------------------------------------------------------------
void TimeCon(u8 *buf)
{
...
//------------------------------------------------------------------------------------------
// 현재 LIVE의 날짜(RTC에서 읽어온 BCD데이터)를 buf에 위치에 맞게 넣어 놓는다.
//------------------------------------------------------------------------------------------
#define ASIA	0
#define USA		1
#define EURO	2
