...
//--------------------------------------------------------------------------------------------------------------------------
static MDIN_ERROR_t MDIN3xx_SetVideoPLL(MDIN_CHIP_ID_t chipId, WORD P, WORD M, WORD S)
{
	//if (vpll_P==P&&vpll_M==M&&vpll_S==S) return MDIN_NO_ERROR;

	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x020, 0, 1, 1)) return MDIN_I2C_ERROR;	// disable PLL
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x02c, P)) return MDIN_I2C_ERROR;		// pre-divider
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x02e, M)) return MDIN_I2C_ERROR;		// post-divider
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x030, S)) return MDIN_I2C_ERROR;		// post-scaler
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x020, 0, 1, 0)) return MDIN_I2C_ERROR;	// enable PLL
...
//--------------------------------------------------------------------------------------------------------------------------
static MDIN_ERROR_t MDIN3xx_SetMemoryPLL(MDIN_CHIP_ID_t chipId, WORD P, WORD M, WORD S)
{
//	Mclk[MHz] = (Fin[MHz]*M)/(P*2^S)
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x020, 1, 1, 1)) return MDIN_I2C_ERROR;	// disable PLL
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x032, P)) return MDIN_I2C_ERROR;		// pre-divider
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x034, (S<<7)|M)) return MDIN_I2C_ERROR;	// post-scaler & post-divider
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x020, 1, 1, 0)) return MDIN_I2C_ERROR;	// enable PLL
	return MDIN_NO_ERROR;
}

//--------------------------------------------------------------------------------------------------------------------------
static MDIN_ERROR_t MDIN3xx_ResetOutSync(MDIN_CHIP_ID_t chipId, WORD delay)
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x099, 0, 13, delay)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x143, 0, 11, delay)) return MDIN_I2C_ERROR;

	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x099, 15, 1, 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x143, 11, 1, 0)) return MDIN_I2C_ERROR;

	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x099, 15, 1, 1)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x143, 11, 1, 1)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetSrcClockPath(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (pINFO->srcPATH == PATH_MAIN_B_AUX_A) {
		if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x03c, 11, 1, 1)) return MDIN_I2C_ERROR; // disable clk_m1
	}
	else {
		if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x03c, 11, 1, 0)) return MDIN_I2C_ERROR; // enable clk_m1
	}
	//===================================================================================================//
	
	// set clk_a from CLK_A or CLK_B
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x047, 4, 1, inA)) return MDIN_I2C_ERROR;

	// set clk_b from CLK_A or CLK_B
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x047, 5, 1, inB)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x048, 8, 2, mode)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x049, 8, 2, mode)) return MDIN_I2C_ERROR;

	// set clk_a2 from clk_a or from clk_a/2 in 4-CH input mode
	mode = (pINFO->dacPATH==DAC_PATH_AUX_4CH)? MDIN_a1_clka_D2P0 : MDIN_a1_clka_D1P0;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x048, 3, 2, mode)) return MDIN_I2C_ERROR;

	// set clk_b2 from clk_b or from clk_b/2 in 4-CH input mode
	mode = (pINFO->dacPATH==DAC_PATH_AUX_4CH)? MDIN_b1_clkb_D2P0 : MDIN_b1_clkb_D1P0;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x049, 3, 2, mode)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x048, 13, 3, mode)) return MDIN_I2C_ERROR;
	...
	if (pINFO->dacPATH==DAC_PATH_AUX_4CH) mode = 4;	// 4-CH input mode
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x049, 13, 3, mode)) return MDIN_I2C_ERROR;
...
//	if (MDINHIF_RegField(MDIN_HOST_ID, 0x049, 2, 1, 0)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x000, 14, 2, mode)) return MDIN_I2C_ERROR;
#endif
...
//	mode |= (MBIT(pSRC->stATTB.attb,MDIN_NEGATIVE_VACT))?	(1<<1) : 0;
	mode |= (MBIT(pSRC->stATTB.attb,MDIN_POSITIVE_HSYNC))?	 0 : (1<<2);	//[2]positive_hsync_a	 //by hungry 2012.03.05
	mode |= (MBIT(pSRC->stATTB.attb,MDIN_POSITIVE_VSYNC))?	 0 : (1<<1);	//[1]positive_vsync_a	 //by hungry 2012.03.05
	mode |= (MBIT(pSRC->stATTB.attb,MDIN_SCANTYPE_PROG))?	(1<<0) : 0;

	// in_sync_ctrl
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x002, 12-nID*8, 4, (mode>>4)&0x0f)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x002,  8-nID*8, 3, (mode>>0)&0x07)) return MDIN_I2C_ERROR;

	// in_fieldid_ctrl
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x001, 0+nID, 1, MBIT(pSRC->fine,MDIN_HIGH_IS_TOPFLD))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x001, 2+nID, 1, MBIT(pSRC->fine,MDIN_FIELDID_INPUT))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01a-nID*0x019, 4, 12, pSRC->stATTB.Htot)) return MDIN_I2C_ERROR;

	// in_format_ctrl
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x000, 8+nID*3, 3, (pSRC->fine&MDIN_YCOFFSET_MASK))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x000, 2+nID, 1, MBIT(pSRC->fine,MDIN_CbCrSWAP_ON))) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x040, 14-nID*2, 2, mode)) return MDIN_I2C_ERROR;

	// vsync_forced_rising - tenbit_input
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x021-nID, 13, 1, RBIT(pSRC->stATTB.attb,MDIN_PRECISION_8))) return MDIN_I2C_ERROR;

	// in_vact_offset_msb
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x003, 0+nID*8, 7, pSRC->offV>>6)) return MDIN_I2C_ERROR;

	// in_act_offset
	mode = ((pSRC->offV&0x3f)<<10)|(pSRC->offH&0x3ff);
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x005-nID, mode)) return MDIN_I2C_ERROR;

	// in_hact_offset_msb
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x008-nID*2, 13, 3, HIBYTE(pSRC->offH)>>2)) return MDIN_I2C_ERROR;

	// in_size_h
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x008-nID*2, 0, 13, pSRC->stATTB.Hact)) return MDIN_I2C_ERROR;

	// in_size_v
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x009-nID*2, pSRC->stATTB.Vact)) return MDIN_I2C_ERROR;

	// in_wr_ch_mask - in_mch_hdeci2_en ==> for 2-HD input mode
	mode = (pINFO->dacPATH==DAC_PATH_AUX_2HD)? 1 : 0;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01b,  8, 2, mode)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01e,  8, 4, mode)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01e,  4, 1, mode)) return MDIN_I2C_ERROR;
...
MDIN_ERROR_t MDIN3xx_SetSrcVideoFrmt(PMDIN_VIDEO_INFO pINFO)
{
	...
	switch (pINFO->srcPATH) {
		...
		case PATH_MAIN_B_AUX_A: case PATH_MAIN_B_AUX_B: case PATH_MAIN_B_AUX_M:
			...
			pSRC->stATTB.attb |=  MDIN_USE_INPORT_B; break;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x040, 3, 1, MBIT(pSRC->stATTB.attb,MDIN_USE_INPORT_A))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x040, 2, 1, 0)) return MDIN_I2C_ERROR;	// fix enable ffc
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x040, 0, 1, 0)) return MDIN_I2C_ERROR;	// fix enable in-buff
	return MDIN_NO_ERROR;
}
...
//		case VIDOUT_1280x720p30:	case VIDOUT_1280x720p25:	case VIDOUT_1280x720p24:
		//case VIDOUT_1280x720p30:	case VIDOUT_1280x720p25:								//by hungry 2012.03.19
...
//	printf("_______________pOUT->frmt = %d \n", pOUT->frmt);
...
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x0a2, 0, 10, mode)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a2, 0, 13, mode)) return MDIN_I2C_ERROR;	// add dither on/off (30Dec2015)
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a0, 0, 10, mode)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x0a3, mode)) return MDIN_I2C_ERROR;

	// digital_blank_data
	mode  = (pOUT->stATTB.attb&MDIN_COLORSPACE_YUV)? 0x1080 : 0x0000; // Y=16, CbCr=128
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x0a4, mode)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a5, 8, 2, mode)) return MDIN_I2C_ERROR;

	// out_mux_ctrl - osd_on_data_to_aux
	mode = (pINFO->dspFLAG&MDIN_AUX_MAINOSD_ON)? 1 : 0;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a5, 7, 1, mode)) return MDIN_I2C_ERROR;

	// out_mux_ctrl - hdmi_data_mode
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a5, 5, 1, 1)) return MDIN_I2C_ERROR;	// fix 30bit
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetSrcVideoCSC(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDINHIF_MultiWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x00a, (PBYTE)&stCSC, sizeof(MDIN_CSCCTRL_INFO))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetOutVideoCSC(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDINHIF_MultiWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x072, (PBYTE)&stCSC, sizeof(MDIN_CSCCTRL_INFO))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
static MDIN_ERROR_t MDIN3xx_SetOutVideoDAC(PMDIN_VIDEO_INFO pINFO)
{
	...
	for (i=0; i<sizeof(MDIN_DACSYNC_CTRL)/2-1; i++) {	// because of dummy data
		...
		if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x0ae, mode)) return MDIN_I2C_ERROR;
		if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x0af, 0x0080|i)) return MDIN_I2C_ERROR;
	}
	return MDIN_NO_ERROR;
}
...
//		if (pAUX->frmt==VIDOUT_1920x1080p50 || pAUX->frmt==VIDOUT_1920x1080p25 || pAUX->frmt==VIDOUT_1920x1080p24) stSYNC.totHS = 1090;
	...
	if (MDINHIF_MultiWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x084, (PBYTE)&stSYNC.posHD,  6)) return MDIN_I2C_ERROR;
	if (MDINHIF_MultiWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x088, (PBYTE)&stSYNC.totHS, 30)) return MDIN_I2C_ERROR;
...
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x083, 6, 1, 1)) return MDIN_I2C_ERROR;	// fix main_skip_dis // deleted on 16Aug2012
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x097, 2, 2, 1)) return MDIN_I2C_ERROR;	// fix vact_ipc_lead
...
//	mode = (1<<15)|(10<<11)|MBIT(pOUT->fine,MDIN_SYNC_FREERUN);
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x098, mode)) return MDIN_I2C_ERROR;	// free-run

	// bg_color_y, cbcr
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x09a, 0x0010)) return MDIN_I2C_ERROR;	// fix black
...
static MDIN_ERROR_t MDIN3xx_SetMFCScaleWind(PMDIN_VIDEO_INFO pINFO)
{
	...
	//printf("[MAIN] stFFC.sw.=%d, stFFC.sh=%d\n", pMFC->stFFC.sw, pMFC->stFFC.sh);
	//printf("[MAIN] stFFC.dw.=%d, stFFC.dh=%d\n", pMFC->stFFC.dw, pMFC->stFFC.dh);

	printf("[MAIN] pMFC->stCRS.sw=%d,pMFC->stCRS.sh=%d\n", pMFC->stCRS.sw, pMFC->stCRS.sh);
	printf("[MAIN] pMFC->stCRS.dw=%d,pMFC->stCRS.dh=%d\n", pMFC->stCRS.dw, pMFC->stCRS.dh);
	printf("[MAIN] pMFC->stORS.sw=%d,pMFC->stORS.sh=%d\n", pMFC->stORS.sw, pMFC->stORS.sh);
	printf("[MAIN] pMFC->stORS.dw=%d,pMFC->stORS.dh=%d\n", pMFC->stORS.dw, pMFC->stORS.dh);
	printf("\n");
#endif

	// in_vact_offset_msb
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x003, 0+nID*8, 7, pMFC->stCUT.y>>6)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x008-nID*2, mode)) return MDIN_I2C_ERROR;

	// in_size_v
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x009-nID*2, pMFC->stCUT.h)) return MDIN_I2C_ERROR;

	// src_posi_h0/v0 - preload src video position
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x030, pMFC->stSRC.x)) return MDIN_I2C_ERROR;	// 12Apr2013
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x031, pMFC->stSRC.y)) return MDIN_I2C_ERROR;	// 12Apr2013
	
	// src_size_h/v, src_posi_h/v
	if (MDINHIF_MultiWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x032, (PBYTE)&pMFC->stSRC, sizeof(MDIN_VIDEO_WINDOW))) return MDIN_I2C_ERROR;

	// win_size_h/v, win_posi_h/v
	if (MDINHIF_MultiWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x036, (PBYTE)&pMFC->stDST, sizeof(MDIN_VIDEO_WINDOW))) return MDIN_I2C_ERROR;

	// src_size_h2
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x03a, pMFC->stFFC.sw)) return MDIN_I2C_ERROR;

	// dst_size_h2
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x03c, pMFC->stFFC.dw)) return MDIN_I2C_ERROR;

	// mem_size_v
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x03d, pMFC->stFFC.dh)) return MDIN_I2C_ERROR;

	// src_size_h/v_cr, dst_size_h/v_cr for uniform mode
	if (MDINHIF_MultiWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x108, (PBYTE)&pMFC->stCRS, sizeof(MDIN_SCALE_FACTOR))) return MDIN_I2C_ERROR;

	// src_size_h/v_or, dst_size_h/v_or for non-uniform mode
	if (MDINHIF_MultiWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x10c, (PBYTE)&pMFC->stORS, sizeof(MDIN_SCALE_FACTOR))) return MDIN_I2C_ERROR;

	// mvf_count_value - mvf_count_value_mo
	mode = ((pMFC->stSRC.w%384)%12)? (pMFC->stSRC.w%384)/12 : (pMFC->stSRC.w%384)/12-1;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x206,  8, 5, MAX(mode,8)&0x1f)) return MDIN_I2C_ERROR;

	// mvf_count_value - mvf_count_value_data
	mode = ((pMFC->stSRC.w%192)%6)? (pMFC->stSRC.w%192)/6 : (pMFC->stSRC.w%192)/6-1;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x206,  0, 5, MAX(mode,8)&0x1f)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x036, 2*pMFC->stDST.w )) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x10a, 2*pMFC->stCRS.dw)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x10e, 2*pMFC->stORS.dw)) return MDIN_I2C_ERROR;
...
//	if (MDIN3xx_GetMFCFilterDone()) {mdinERR = 6; return MDIN_TIMEOUT_ERROR;}
	...
	if (MDINHIF_MultiWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x13b, (PBYTE)buff, 10)) return MDIN_I2C_ERROR;
...
static MDIN_ERROR_t MDIN3xx_SetMFCScaleCtrl(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x100, 8, 8, (2<<6)|(2<<4)|mode)) return MDIN_I2C_ERROR;
//	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x100, 7, 1, 1)) return MDIN_I2C_ERROR;		// fix blank update
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x100, 7, 1, 0)) return MDIN_I2C_ERROR;		// fix immediately
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x100, 2, 1, 0)) return MDIN_I2C_ERROR;		// enable MFC filter
...
//	mode = ((pMFC->stFFC.dh>pMFC->stMEM.h)&&((mode&3)==3))? 1 : 0;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x100, 1, 1, mode)) return MDIN_I2C_ERROR;
...
//	if (pSRC->stATTB.attb&MDIN_SCANTYPE_PROG) pSRC->stATTB.attb &= ~MDIN_PRECISION_8;	// set 10-bit processing, delete on 20Mar2012
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x040,  8, 1, RBIT(pSRC->stATTB.attb,MDIN_PRECISION_8))) return MDIN_I2C_ERROR;

	// common_mode_at_IPC - mvfw_count_en
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x205,  8, 1, MBIT(pSRC->stATTB.attb,MDIN_PRECISION_8))) return MDIN_I2C_ERROR;

	// internal_mode_set_1 - mvfr_count_en
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x208, 14, 1, MBIT(pSRC->stATTB.attb,MDIN_PRECISION_8))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
static MDIN_ERROR_t MDIN3xx_SetDeinterCtrl(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x040, 6, 1, MBIT(pIPC->attb,MDIN_DEINT_MFC_BUFF))) return MDIN_I2C_ERROR;

	// mfc_control2 - fix BNR filtering only
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x101, 0, 3, (1<<1)|MBIT(pIPC->attb,MDIN_DEINT_MFC_BUFF))) return MDIN_I2C_ERROR;

	// common_mode_at_IPC - v_posi_half_en, v_size_half_en
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x205, 7, 1, MBIT(pIPC->attb,MDIN_DEINT_IPC_PROC))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x205, 6, 1, MBIT(pIPC->attb,MDIN_DEINT_IPC_PROC))) return MDIN_I2C_ERROR;

	// common_mode_at_IPC - deinterlace_en
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x205, 4, 1, MBIT(pIPC->attb,MDIN_DEINT_IPC_PROC))) return MDIN_I2C_ERROR;

	// common_mode_at_IPC - ycbcr444_en
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x205, 1, 1, MBIT(pIPC->attb,MDIN_DEINT_444_PROC))) return MDIN_I2C_ERROR;

	// internal_mode_set_1 - ipc_2tab_mode
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x208, 13, 1, MBIT(pIPC->attb,MDIN_DEINT_IPC_2TAB))) return MDIN_I2C_ERROR;

	// internal_mode_set_1 - block_match_en, block_match_adp_en
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x208, 11, 1, MBIT(pIPC->attb,MDIN_DEINT_IPC_PROC))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x208, 10, 1, MBIT(pIPC->attb,MDIN_DEINT_IPC_PROC))) return MDIN_I2C_ERROR;

	// 8bit_motion_thres - deint_thres
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x211, 8, 8, 18)) return MDIN_I2C_ERROR; // fix deint_thres

	// caption_v_posi
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x251, pMFC->stMEM.h*3/4)) return MDIN_I2C_ERROR;
...
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x252,  0, 2, (pIPC->attb&MDIN_DEINT_p5_SCALE)? 1 : 3)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x26d, 7, 1, MBIT(pIPC->attb,MDIN_DEINT_IPC_PROC))) return MDIN_I2C_ERROR;
...
//	}
	...
	if ((pIPC->attb&MDIN_DEINT_HD_1080i)&&
		...
		if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x249, 12, 2, 2)) return MDIN_I2C_ERROR; // c intra
		if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x252,  0, 2, 1)) return MDIN_I2C_ERROR;
	}
	else {
		if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x249, 12, 2, 1)) return MDIN_I2C_ERROR; // c adaptive
		if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x252,  0, 2, 3)) return MDIN_I2C_ERROR;
	}

	// progressive ipc mode - 20Mar2012
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x100,  3, 1, (pIPC->attb&MDIN_DEINT_PROG_IPC)? 1 : 0)) return MDIN_I2C_ERROR; // mfc_interlaced_input
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x208, 15, 1, (pIPC->attb&MDIN_DEINT_PROG_IPC)? 1 : 0)) return MDIN_I2C_ERROR; // enable p-ipc mode
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x083,  5, 1, (pIPC->attb&MDIN_DEINT_PROG_IPC)? 1 : 0)) return MDIN_I2C_ERROR; // main_repeat_dis

	// modified on 17Oct2013
	if (pIPC->attb&MDIN_DEINT_IPC_PROC)	{ // if interlace input
		if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x083,  6, 1, 1)) return MDIN_I2C_ERROR;	// fix main_skip_dis
	}
	else {			// if progressive input
		if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x083,  6, 1, (pIPC->fine&MDIN_DEINT_FRC_DOWN)? 0 : 1)) return MDIN_I2C_ERROR; // main_skip_dis, added on 28Dec2012
	}

	// film22_be_ctrl_reg2
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x258, 0, 8, (pIPC->attb&MDIN_DEINT_HD_1080i)? 24 : 20)) return MDIN_I2C_ERROR;

	// film22_be_ctrl_reg3
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x25a, 0, 8, (pIPC->attb&MDIN_DEINT_HD_1080i)? 30 : 40)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x040, 4, 1, MBIT(pIPC->fine,MDIN_PROCESS_444))) return MDIN_I2C_ERROR;
...
static MDIN_ERROR_t MDIN3xx_SetMemoryMap(PMDIN_VIDEO_INFO pINFO, BYTE nID, BYTE num, WORD addr)
{
	...
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1c1+nID, addr)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1c9+(nID%4), LOWORD(rpf))) return MDIN_I2C_ERROR;

	mode = ((bpp==8)? 0 : ((bpp==10)? 1 : ((bpp==4)? 2 : 3)))<<10 | LOWORD(cpl);
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1cd+(nID%4), mode)) return MDIN_I2C_ERROR;
...
static MDIN_ERROR_t MDIN3xx_SetFrameBuffer(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01e, 0, 2, 3)) return MDIN_I2C_ERROR;	// fix manu mode

	// in_frame_cfg2
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x01f, MAKEWORD(numY,numC))) return MDIN_I2C_ERROR;

	// internal_mode_set_2 - y_num_frame, c_num_frame
	mode = MAKEBYTE(HI4BIT(LOBYTE(numY)),HI4BIT(LOBYTE(numC)));
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x209, 0, 8, mode)) return MDIN_I2C_ERROR;

	// aux_frame_ptr_ctrl - aux_frame_config, aux_num_frames, aux_frame_delay
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x144, 11, 5, 0)) return MDIN_I2C_ERROR; // fix auto mode

	// mem_config
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x1c0, 12, 4, (1<<2))) return MDIN_I2C_ERROR;
...
//	pMAP->Y_m += pMAP->Ynr;					// fbuf0y
	...
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1d1, mode)) return MDIN_I2C_ERROR;

	// req_mapping_2
	mode = (1<<12)|(1<<8);				// mvfw_mh=1, mvfr_mh=1
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1d2, mode)) return MDIN_I2C_ERROR;

	// req_mapping_3
	mode = (0<<12)|((8+0)<<4);			// input_y=0, input_c=0
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1d3, mode)) return MDIN_I2C_ERROR;

	// req_mapping_4
	mode = (2<<12)|(2<<8)|((8+2)<<4)|((8+2)<<0);	// aux_y=2, aux_c=2
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1d4, mode)) return MDIN_I2C_ERROR;

	// f_count_1 ==> for 4-CH input mode, 2-HD input mode
	mode = (pINFO->dacPATH==DAC_PATH_AUX_4CH)? 0x8220 : 0x4120;
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1dd, mode)) return MDIN_I2C_ERROR;

	// f_count_2 ==> for 4-CH input mode, 2-HD input mode
	mode = (pINFO->dacPATH==DAC_PATH_AUX_4CH)? 0x8020 : 0x3020;
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1de, mode)) return MDIN_I2C_ERROR;

	// ad_start_row, ad_end_row
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1f8, fbADDR)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1f9, fbADDR+15)) return MDIN_I2C_ERROR;
	fbADDR += 16;
...
MDIN_ERROR_t MDIN3xx_EnableWriteFRMB(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if(OnOff == OFF)	
	{
		if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x040, 5, 1, (OnOff)? ON : OFF)) return MDIN_I2C_ERROR;
	}
	
	// main_freeze
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x040, 1, 1, (OnOff)? frez_M : 1)) return MDIN_I2C_ERROR;
	...
	//else 
	{
		if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x142, 0, 1, OFF)) return MDIN_I2C_ERROR;
	}

	
	// aux_freeze
	ctrl = MBIT(pINFO->dspFLAG, MDIN_AUX_FREEZE_ON);
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x142, 1, 1, (OnOff)? ctrl : ON)) return MDIN_I2C_ERROR;

	// aux_pip
	ctrl = (pINFO->dacPATH==DAC_PATH_MAIN_PIP)? 1 : 0;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x144, 6, 1, (OnOff)? ctrl : OFF)) return MDIN_I2C_ERROR;

	// ipc_top_en
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x205, 3, 1, (OnOff)? ON : OFF)) return MDIN_I2C_ERROR;
...
static MDIN_ERROR_t MDIN3xx_Set4CHProcess(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDINHIF_RegRead(pINFO->chipId, MDIN_LOCAL_ID, 0x0fd, &rVal)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x14a, 0, 8, rVal)) return MDIN_I2C_ERROR;
...
MDIN_ERROR_t MDIN3xx_SetSrcCompHsync(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x002, 6+nID*8, 1, MBIT(pSRC->fine,MDIN_HACT_IS_CSYNC))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetSrcInputFieldID(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x001, 2+nID, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetSrcHighTopFieldID(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x001, 0+nID, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetSrcYCOffset(PMDIN_VIDEO_INFO pINFO, MDIN_YC_OFFSET_t val)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x000, 8+nID*3, 3, val)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetSrcCbCrSwap(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x000, 2+nID, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetSrcHACTPolarity(PMDIN_VIDEO_INFO pINFO, BOOL edge)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x002, 10-nID*8, 1, MBIT(edge,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetSrcVACTPolarity(PMDIN_VIDEO_INFO pINFO, BOOL edge)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x002, 9-nID*8, 1, MBIT(edge,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetSrcOffset(PMDIN_VIDEO_INFO pINFO, WORD offH, WORD offV)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x003, 0+nID*8, 7, offV>>6)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x005-nID, ((offV&0x3f)<<10)|(offH&0x3ff))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x008-nID*2, 13, 3, HIBYTE(offH)>>2)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetOutCbCrSwap(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a2, 4, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetOutPbPrSwap(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a3, 4, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetOutDEPinMode(PMDIN_VIDEO_INFO pINFO, MDIN_DEOUT_MODE_t mode)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a0, 8, 2, mode)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetOutHSPinMode(PMDIN_VIDEO_INFO pINFO, MDIN_HSOUT_MODE_t mode)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a0, 2, 2, mode)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetOutVSPinMode(PMDIN_VIDEO_INFO pINFO, MDIN_VSOUT_MODE_t mode)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a0, 0, 2, mode)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetOutDEPolarity(PMDIN_VIDEO_INFO pINFO, BOOL edge)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a0, 7, 1, MBIT(edge,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetOutHSPolarity(PMDIN_VIDEO_INFO pINFO, BOOL edge)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a0, 5, 1, MBIT(edge,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetOutVSPolarity(PMDIN_VIDEO_INFO pINFO, BOOL edge)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a0, 4, 1, MBIT(edge,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x048, 10, 3, delay)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x049, 10, 3, delay)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x024, 0, 3, delay)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x024, 4, 3, delay)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x047, 4, 1, src)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x047, 5, 1, src)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x048, 8, 2, src)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x048, 3, 2, src)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x049, 8, 2, src)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x049, 3, 2, src)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x048, 13, 3, src)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x049, 13, 3, src)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetFrontNRFilterCoef(MDIN_CHIP_ID_t chipId, PMDIN_FNRFILT_COEF pCoef)
{
	...
	if (MDINHIF_MultiWrite(chipId, MDIN_LOCAL_ID, 0x022, (PBYTE)pCoef->y, 16)) return MDIN_I2C_ERROR;
	if (MDINHIF_MultiWrite(chipId, MDIN_LOCAL_ID, 0x02c, (PBYTE)pCoef->c,  8)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x02a, 3, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x02a, 0, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetPeakingFilterCoef(MDIN_CHIP_ID_t chipId, PMDIN_FNRFILT_COEF pCoef)
{
	...
	if (MDINHIF_MultiWrite(chipId, MDIN_LOCAL_ID, 0x120, (PBYTE)pCoef, 16)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x129, 8, 8, val)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x128, 0, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x063, (OnOff)? 0x02f9 : 0)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x065, (OnOff)? 0x02f9 : 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x067, (OnOff)? 0x02f9 : 0)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetColorEnFilterCoef(MDIN_CHIP_ID_t chipId, PMDIN_COLOREN_COEF pCoef)
{
	...
	if (MDINHIF_MultiWrite(chipId, MDIN_LOCAL_ID, 0x068, (PBYTE)pCoef, 8)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x06c, 0, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetBlockNRFilterCoef(MDIN_CHIP_ID_t chipId, PMDIN_BNRFILT_COEF pCoef)
{
	...
	if (MDINHIF_MultiWrite(chipId, MDIN_LOCAL_ID, 0x12a, (PBYTE)pCoef, 8)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x12e, (OnOff)? 5 : 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x208, 3, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetMosquitFilterCoef(MDIN_CHIP_ID_t chipId, PMDIN_MOSQUIT_COEF pCoef)
{
	...
	if (MDINHIF_MultiWrite(chipId, MDIN_LOCAL_ID, 0x050, (PBYTE)pCoef, 18)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x058, 0, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetColorTonFilterCoef(MDIN_CHIP_ID_t chipId, PMDIN_COLORTON_COEF pCoef)
{
	...
	if (MDINHIF_MultiWrite(chipId, MDIN_LOCAL_ID, 0x3f0, (PBYTE)pCoef, 10)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x3f3, 10, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetBWExtensionPoint(MDIN_CHIP_ID_t chipId, PMDIN_BWPOINT_COEF pCoef)
{
	...
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x3d0, MAKEWORD(pCoef->src[0],pCoef->src[1]))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x3d1, MAKEWORD(pCoef->src[2],pCoef->src[3]))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x3d2, MAKEWORD(pCoef->out[0],pCoef->out[1]))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x3d3, MAKEWORD(pCoef->out[2],pCoef->out[3]))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x3c0, 0, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x208, 10, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDIN3xx_GetDeviceID(chipId, &GetDID)) return MDIN_I2C_ERROR;			// get device-ID
...
//	if (MDIN3xx_SetMemoryPLL((GetMEM)? 1 : 4, (GetMEM)? 12 : 59, 0)) return MDIN_I2C_ERROR;
	...
	if (MDIN3xx_SetMemoryPLL(chipId, 1, (GetMEM)? 12 : 14, 0)) return MDIN_I2C_ERROR;
#else
	if (MDIN3xx_SetMemoryPLL(chipId, 1, (GetMEM)? 12 : 15, 0)) return MDIN_I2C_ERROR;
#endif

	if (MDINDLY_mSec(1)) return MDIN_I2C_ERROR;		// delay 1ms

	if (MDIN3xx_GetVersionID(chipId, &mdinREV)) return MDIN_I2C_ERROR;			// get version-ID

	// ddr_init
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x094, 0x072f)) return MDIN_I2C_ERROR;	// dfi isft
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x090, 0, 1, 1)) return MDIN_I2C_ERROR;	// dfi ireset
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x090, 1, 1, 1)) return MDIN_I2C_ERROR;	// dfi iusrst
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x090, 2, 1, 1)) return MDIN_I2C_ERROR;	// dfi idllrst
	if (MDINDLY_mSec(1)) return MDIN_I2C_ERROR;		// delay 1ms

#if	defined(SYSTEM_USE_MDIN380)
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x1a7, 0x4004)) return MDIN_I2C_ERROR;	// ddr initialize
#else
	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x1a7, 0x4008)) return MDIN_I2C_ERROR;	// ddr initialize
#endif
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID,  0x093, 0x2f80)) return MDIN_I2C_ERROR;	// dfi latency init
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x1a7, 13, 1,  1)) return MDIN_I2C_ERROR;	// ddr access enable

	// bank_2kb, ddr_7bit, mem_7bit, audio_8bit, osd_7bit, axosd_7bit
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID,  0x005,  8, 1, ~GetMEM&1)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x1a7, 12, 1,  GetMEM&1)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x1c0, 11, 1,  GetMEM&1)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x1fb,  7, 1, ~GetMEM&1)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x300,  1, 1,  GetMEM&1)) return MDIN_I2C_ERROR; //osd_col_addr_7bit
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x2a0,  6, 1,  GetMEM&1)) return MDIN_I2C_ERROR;

	// ddr timing tune
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x1a3, 11, 2, (GetMEM)? 0 : 3)) return MDIN_I2C_ERROR;

	// hdmi_clk_ctrl1 - audio_delay_bypass
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID,  0x03e, 15, 1, 1)) return MDIN_I2C_ERROR;

	// arbiter_ctrl - refresh_prog_count
	GetPRI = (GetDID==MDIN_DID_325A)? 0x1b84 : (GetDID==MDIN_DID_380)? 0x3b84 : 0x0004;
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x1a0, GetPRI)) return MDIN_I2C_ERROR;	// fix
//	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x1a0, 0x0004)) return MDIN_I2C_ERROR;	// fix
	...
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x1a1, GetPRI)) return MDIN_I2C_ERROR;	// fix
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x1a2, GetSTV)) return MDIN_I2C_ERROR;	// fix

	// fcmc_arb_ctrl1
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x1d5, 0xeac2)) return MDIN_I2C_ERROR;	// fix
...
//	if (MDINHIF_RegField(MDIN_LOCAL_ID, 0x1a7, 0, 12, 4)) return MDIN_I2C_ERROR;	// fix
	...
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID,  0x005, 0, 8, 16)) return MDIN_I2C_ERROR;	// fix
...
#else
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x005, 11, 2, 3)) return MDIN_I2C_ERROR;	// fix big-endian
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x006, 14, 2, 3)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x012, 0x0003)) return MDIN_I2C_ERROR;	// fix big-endian
#endif

	// osd_endian_mode
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x300, 2, 1, 0)) return MDIN_I2C_ERROR;	// fix big-endian
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x300,12, 1, 1)) return MDIN_I2C_ERROR;	// fix BG-BOX ON

	// axosd_endian_mode
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x2a0, 7, 1, 0)) return MDIN_I2C_ERROR;	// fix big-endian

	// sdram_32bit
//	if (MDINHIF_RegWrite(MDIN_LOCAL_ID, 0x3fe, 0x0000)) return MDIN_I2C_ERROR;	// fix 64bit

	// default frame buffer map
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x040, 1, 1, 1)) return MDIN_I2C_ERROR;	// main_freeze
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x142, 1, 1, 1)) return MDIN_I2C_ERROR;	// aux_freeze
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x205, 3, 1, 0)) return MDIN_I2C_ERROR;	// ipc_top_off

	// fix in_wr_posi_h bug
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x040,  8, 1, 1)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x01c, 10, 1, 1)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x01c, 10, 1, 0)) return MDIN_I2C_ERROR;
	...
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x01e, GetPAD)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x03c, GetCLK)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x045, 13, 1, GetENC)) return MDIN_I2C_ERROR;
...
{
	if (MDINHIF_RegRead(chipId, MDIN_HOST_ID, 0x000, pID)) return MDIN_I2C_ERROR;
...
{
	if (MDINHIF_RegRead(chipId, MDIN_HOST_ID, 0x004, pID)) return MDIN_I2C_ERROR;

#if __MDIN3xx_DBGPRT__ == 1
	printf("MDIN3xx_GetDeviceID = 0x%04X\n", *pID);
#endif
...
{
	if (MDINHIF_RegRead(chipId, MDIN_HOST_ID, 0x007, pID)) return MDIN_I2C_ERROR;
	*pID &= 0x0001;
...
{
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x072, 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x072, 1)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x1a0,  6, 1, 1)) return MDIN_I2C_ERROR;	// disable refresh
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x1a7, 13, 1, 0)) return MDIN_I2C_ERROR;	// disable access

	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x070, 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x070, 1)) return MDIN_I2C_ERROR;

	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x1a7, 13, 1, 1)) return MDIN_I2C_ERROR;	// enable access
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x1a0,  6, 1, 0)) return MDIN_I2C_ERROR;	// enable refresh
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x00e, irq, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
//	if (MDINHIF_RegField(MDIN_HOST_ID, 0x016, irq, 1, 0)) return MDIN_I2C_ERROR;	// fix edge detect
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x016, 0x0080)) return MDIN_I2C_ERROR;		// fix edge detect
	if (MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x08e, 11, 1, 0)) return MDIN_I2C_ERROR;		// fix active low
	return MDIN_NO_ERROR;
}
...
{
	WORD irq;
	if (MDINHIF_RegRead(chipId, MDIN_HOST_ID, 0x010, &irq)) return MDIN_I2C_ERROR;
	GetIRQ |= irq;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetPriorityHIF(MDIN_CHIP_ID_t chipId, MDIN_PRIORITY_t mode)
{
	...
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x1a1, val)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x03a, (src==MDIN_PLL_SOURCE_HACT)? 1 : 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x02a, (src==MDIN_PLL_SOURCE_HACT)? 0 : src)) return MDIN_I2C_ERROR;
	return MDIN3xx_SoftReset(chipId);
}
...
MDIN_ERROR_t MDIN3xx_EnableOutputPAD(MDIN_CHIP_ID_t chipId, MDIN_PAD_OUT_t id, BOOL OnOff)
{
	if (id==MDIN_PAD_ALL_OUT) {
		if (MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x01e, (OnOff)? GetPAD : 0x1f)) return MDIN_I2C_ERROR;
		return MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x045, 13, 1, (OnOff)? GetENC : 1);
	}
	...
	return MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x01e, id, 1, RBIT(OnOff,1));
}
...
MDIN_ERROR_t MDIN3xx_EnableClockDrive(MDIN_CHIP_ID_t chipId, MDIN_CLK_DRV_t id, BOOL OnOff)
{
	if (id==MDIN_CLK_DRV_ALL) {
		return MDINHIF_RegWrite(chipId, MDIN_HOST_ID, 0x03c, (OnOff)? GetCLK : 0xffff);
	}

	return MDINHIF_RegField(chipId, MDIN_HOST_ID, 0x03c, id, 1, RBIT(OnOff,1));
}

//--------------------------------------------------------------------------------------------------------------------------
MDIN_ERROR_t MDIN3xx_EnableMainDisplay(MDIN_CHIP_ID_t chipId, BOOL OnOff)
{
	//printf("MDIN3xx_EnableMainDisplay(%d)\n",OnOff);
	
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x040, 5, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	if (OnOff==ON)	MDINDLY_mSec(80);	// delay 80ms when ON

	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x043, 1, 1, RBIT(OnOff,1))) return MDIN_I2C_ERROR;
...
{
	frez_M = MBIT(OnOff,1);
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x040, 1, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (OnOff==ON)	MDINDLY_mSec(80);	// delay 80ms when ON
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x144, 6, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	if (OnOff==OFF)	MDINDLY_mSec(40);	// delay 40ms when OFF
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_EnableAuxDisplay(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x142, 0, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	if (OnOff==ON)	MDINDLY_mSec(80);	// delay 80ms when ON

	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x14c, 9, 1, RBIT(OnOff,1))) return MDIN_I2C_ERROR;
	if (OnOff==OFF)	MDINDLY_mSec(40);	// delay 40ms when OFF
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_EnableAuxFreeze(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x142, 1, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_EnableAuxWithMainOSD(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x0a5, 7, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x000, 4, 3, mode)) return MDIN_I2C_ERROR;	// in data map mode
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_SetDIGOutMapMode(MDIN_CHIP_ID_t chipId, MDIN_DIG_OUT_MAP_t mode)
{
	...
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x141, 8, 4, LO4BIT(mode))) return MDIN_I2C_ERROR;	// aux dig out mode
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x141, 7, 1, HI4BIT(mode))) return MDIN_I2C_ERROR;	// main/aux select
	return MDIN_NO_ERROR;
}
#endif
...
{
//	printf("MDIN3xx_SetHostDataMapMode()\n");
...
MDIN_ERROR_t MDIN3xx_SetFormat4CHID(PMDIN_VIDEO_INFO pINFO, MDIN_4CHID_FORMAT_t mode)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x000, 14, 2, mode)) return MDIN_I2C_ERROR;	// chipID read mode
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_FrameMemoryReAlloc(PMDIN_VIDEO_INFO pINFO)
{
	...
	// ad_start_row, ad_end_row (for hdmi audio)
	if (MDINHIF_RegRead(pINFO->chipId, MDIN_LOCAL_ID, 0x1fb, &AdelayState)) return MDIN_I2C_ERROR;	// read current status of audio delay function
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x1fb, 0, 2, 0)) return MDIN_I2C_ERROR;		// disable audio delay & disable audio output
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1f8, fbADDR)) return MDIN_I2C_ERROR;
#if	defined(SYSTEM_USE_MDIN380)
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1f9, fbADDR+48)) return MDIN_I2C_ERROR; // for audio delay buffer
#elif defined(SYSTEM_USE_MDIN340)
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x1f9, fbADDR+96)) return MDIN_I2C_ERROR; // for audio delay buffer
#endif
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x1fb, 0, 2, AdelayState&0x03)) return MDIN_I2C_ERROR;	// enable audio delay & enable audio output
	return MDIN_NO_ERROR;
}
...
MDIN_ERROR_t MDIN3xx_EnableMirrorH(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	UnitSize = (pSRC->stATTB.attb&MDIN_PRECISION_8)? 256 : 192;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01c, 0, 11, (OnOff)? (pMFC->stFFC.dw/UnitSize)*UnitSize : 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01c, 13, 1, RBIT(OnOff,1))) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01c, 15, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;

//	BuffSize = ROUNDUP(pMFC->stFFC.dw, UnitSize)*UnitSize;
	BuffSize = ((pMFC->stFFC.dw/UnitSize)+1)*UnitSize;
	RemainSize = BuffSize - pMFC->stFFC.dw;
	if (!(pSRC->stATTB.attb&MDIN_PRECISION_8)) RemainSize = (ROUNDUP(RemainSize, 6)*6); // 'src_posi_h' should be multiple of 6, when 10bit process mode
	if (MDINHIF_RegWrite(pINFO->chipId, MDIN_LOCAL_ID, 0x034, (OnOff)? RemainSize : 0)) return MDIN_I2C_ERROR;
...
MDIN_ERROR_t MDIN3xx_EnableMirrorV(PMDIN_VIDEO_INFO pINFO, BOOL OnOff)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01d, 0, 11, wVal)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x01c, 14, 1, MBIT(OnOff,1))) return MDIN_I2C_ERROR;

	// inverse input field-id
	wVal = MBIT(pSRC->fine,MDIN_HIGH_IS_TOPFLD);	wVal = (OnOff)? ~wVal : wVal;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x001, 0+nID, 1, wVal&0x01)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
/*
//--------------------------------------------------------------------------------------------------------------------------
...
static MDIN_ERROR_t MDIN3xx_SetSrcVideoRestore(PMDIN_VIDEO_INFO pINFO, MDIN_IN_TEST_t mode)
{
	...
	// in_test_ptrn
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x042, 0, 6, 0)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x042, 7, 1, 0)) return MDIN_I2C_ERROR;

	// clk_m1_sel or clk_m2_sel
	if (MDINHIF_RegRead(pINFO->chipId, MDIN_LOCAL_ID, 0x002, &rVal)) return MDIN_I2C_ERROR;
	rVal = (nID)? ((rVal>>4)&3) : ((rVal>>12)&3);	rVal = (rVal==2)? 1 : 0;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x049-nID, 13, 3, rVal)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
static MDIN_ERROR_t MDIN3xx_SetSrcVideoPattern(PMDIN_VIDEO_INFO pINFO, MDIN_IN_TEST_t mode)
{
	...
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x042, 0, 6, rVal)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegField(pINFO->chipId, MDIN_LOCAL_ID, 0x042, 7, 1, 1)) return MDIN_I2C_ERROR;

	// clk_m1_sel or clk_m2_sel
	if (MDINHIF_RegField(pINFO->chipId, MDIN_HOST_ID, 0x049-nID, 13, 3, 0)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
...
{
	if (MDINHIF_RegField(chipId, MDIN_LOCAL_ID, 0x043, 2, 4, mode)) return MDIN_I2C_ERROR;
	return MDIN_NO_ERROR;
}
/*
//--------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------
static MDIN_ERROR_t MDIN3xx_GetSyncDone(MDIN_CHIP_ID_t chipId)
{
	WORD rVal = 0, count = 100;

	while (count&&(rVal==0)) {
		if (MDINHIF_RegRead(chipId, MDIN_LOCAL_ID, 0x0d0, &rVal)) return MDIN_I2C_ERROR;
...
MDIN_ERROR_t MDIN3xx_GetSyncInfo(MDIN_CHIP_ID_t chipId, PMDIN_AUTOSYNC_INFO pAUTO, WORD hclk)
{
	...
	if (MDINHIF_RegRead(chipId, MDIN_LOCAL_ID, 0x042, rBuff)) return MDIN_I2C_ERROR;
	if (rBuff[0]&0x0080) pAUTO->err |= AUTO_SYNC_SRC_PTRN;
	if (rBuff[0]&0x0080) return MDIN_NO_ERROR;

	// enable auto-detection
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x0d0, 0x1001)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x0d9, 0x0001)) return MDIN_I2C_ERROR;

	if (MDIN3xx_GetSyncDone(chipId)) return MDIN_TIMEOUT_ERROR;	// check done flag

	// disable auto-detection
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x0d0, 0x1000)) return MDIN_I2C_ERROR;
	if (MDINHIF_RegWrite(chipId, MDIN_LOCAL_ID, 0x0d9, 0x0000)) return MDIN_I2C_ERROR;

	// get sync_chg and sync_lost
	if (MDINHIF_RegRead(chipId, MDIN_LOCAL_ID, 0xe0, rBuff)) return MDIN_I2C_ERROR;
	pAUTO->sync = (nID)? HIBYTE(rBuff[0])&3 : LOBYTE(rBuff[0])&3;

	if (MDINHIF_MultiRead(chipId, MDIN_LOCAL_ID, 0xe4+nID, (PBYTE)rBuff, 12)) return MDIN_I2C_ERROR;
