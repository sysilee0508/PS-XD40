...
MDIN_ERROR_t MDINBUS_RegWrite(BYTE nID, DWORD rAddr, WORD wData)
{
	...
	//printf("MDINBUS_RegWrite(%d, 0x%4x, 0x%4x);\n",nID, rAddr ,wData);
...
MDIN_ERROR_t MDINBUS_RegRead(BYTE nID, DWORD rAddr, PWORD rData)
{
	...
	//printf("MDINBUS_RegRead(%d, 0x%04x, 0x%4x);\n",nID, rAddr ,*rData);
...
/*
static void MDINBUS_Write(BYTE nID, WORD rAddr, PBYTE pBuff, WORD bytes) //8bit Mux
{
	WORD i;
	...
	if (nID==MDIN_SDRAM_ID) 
	{
		for (i=0; i<bytes; i++) 
		{
			MDIN3xx_CS_LOW;
			MDIN3xx_ALE_HIGH;
			MDIN3xx_ADDR_DATA_OUTPUT(rAddr+i); 		//addr
			MDIN3xx_ALE_LOW;
			MDIN3xx_ADDR_DATA_OUTPUT(pBuff[i]); 	//data
			//MDIN3xx_ADDR_DATA_OUTPUT(LOBYTE(((PBYTE)pBuff)[i])); 	//data
			MDIN3xx_WR_STROBE_LOW;
			while(!MDIN3xx_WAIT_INPUT);
			//CS_ALT(50);
			MDIN3xx_WR_STROBE_HIGH;
			MDIN3xx_CS_HIGH;
		}
	}
	else 
	{
		...
		//for (i=0; i<bytes/2; i++)
		{
			MDIN3xx_CS_LOW;
			MDIN3xx_ALE_HIGH;
			MDIN3xx_ADDR_DATA_OUTPUT(rAddr+i+1); 	//addr
			//MDIN3xx_ADDR_DATA_OUTPUT(rAddr+i*2+1); 	//addr
			MDIN3xx_ALE_LOW;
			MDIN3xx_ADDR_DATA_OUTPUT(pBuff[i+1]); 	//data
			//MDIN3xx_ADDR_DATA_OUTPUT(HIBYTE(((PWORD)pBuff)[i])); 	//data
			MDIN3xx_WR_STROBE_LOW;
			while(!MDIN3xx_WAIT_INPUT);
			//CS_ALT(50);
			MDIN3xx_WR_STROBE_HIGH;
			...
			MDIN3xx_ALE_HIGH;
			MDIN3xx_ADDR_DATA_OUTPUT(rAddr+i); 		//addr
			//MDIN3xx_ADDR_DATA_OUTPUT(rAddr+i*2+0); 		//addr
			MDIN3xx_ALE_LOW;
			MDIN3xx_ADDR_DATA_OUTPUT(pBuff[i]); 	//data
			//MDIN3xx_ADDR_DATA_OUTPUT(LOBYTE(((PWORD)pBuff)[i])); 	//data
			MDIN3xx_WR_STROBE_LOW;
			while(!MDIN3xx_WAIT_INPUT);
			//CS_ALT(50);
			MDIN3xx_WR_STROBE_HIGH;
	...
	//printf(" nID:%02X, Addr:%04X, rAddr:%04X, pBuff:%04X, bytes:%04X\n", nID, rAddr/2, rAddr, ((PWORD)pBuff)[0], bytes);
}*/
...
//--------------------------------------------------------------------------------------------------------------------------
/*static void MDINBUS_Read(BYTE nID, WORD rAddr, PBYTE pBuff, WORD bytes) //8bit Mux
{
	WORD i;
	...
	if (nID==MDIN_SDRAM_ID) 
	{
		for (i=0; i<bytes; i++) 
		{
			...
			MDIN3xx_ADDR_DATA_OUTPUT(rAddr+i); //addr
			MDIN3xx_ALE_LOW;
			MDIN3xx_RD_STROBE_LOW;
			MDIN3xx_ADDR_DATA_INPUT_MODE;
			while(!MDIN3xx_WAIT_INPUT);
			//CS_ALT(50);
			pBuff[i] = MDIN3xx_ADDR_DATA_INPUT; //data
			//((PBYTE)pBuff)[i] = MDIN3xx_ADDR_DATA_INPUT; //data
			MDIN3xx_RD_STROBE_HIGH;
			MDIN3xx_CS_HIGH;
			MDIN3xx_ADDR_DATA_OUTPUT_MODE;		
		}
	}
	else 
	{
		...
		//for (i=0; i<bytes/2; i++)
		{
			...
			while(!MDIN3xx_WAIT_INPUT);
			//CS_ALT(50);
			pBuff[i+1] = MDIN3xx_ADDR_DATA_INPUT; //data
			//((PWORD)pBuff)[i] = MDIN3xx_ADDR_DATA_INPUT<<8; //data
			MDIN3xx_RD_STROBE_HIGH;
			MDIN3xx_CS_HIGH;
			MDIN3xx_ADDR_DATA_OUTPUT_MODE;
			...
			//CS_ALT(50);
			pBuff[i] = MDIN3xx_ADDR_DATA_INPUT; //data
			//((PWORD)pBuff)[i] |= MDIN3xx_ADDR_DATA_INPUT; //data
			MDIN3xx_RD_STROBE_HIGH;
			MDIN3xx_CS_HIGH;
			MDIN3xx_ADDR_DATA_OUTPUT_MODE;
		}
...
//		while (GET_MDIN_DREQ()==HIGH);			// wait for /DREQ	   //by hungry 2012.02.08
