...
#if SYSTEM_USE_HTX_HDCP == 1								   //API v0.31(2012.05.02)
...
#if __MDINHTX_DBGPRT__ == 1
	if (count==0) printf("MDDC bus not relinquished.\n");
#endif
...
#if __MDINHTX_DBGPRT__ == 1
	if (err) printf("DDC Write method is failure. No ACK.\n");
#endif
...
{
#if __MDINHTX_DBGPRT__ == 1
	printf(" H Active %d ", MAKEWORD(HI4BIT(pBuff[2]),pBuff[0]));
	printf(" H Blank %d ", MAKEWORD(LO4BIT(pBuff[2]),pBuff[1]));
	printf(" V Active %d ", MAKEWORD(HI4BIT(pBuff[5]),pBuff[3]));
	printf(" V Blank %d ", MAKEWORD(LO4BIT(pBuff[5]),pBuff[4]));
#endif
}
...
{
#if __MDINHTX_DBGPRT__ == 1
	printf(" H Sync Offset %d", MAKEWORD(HI4BIT(pBuff[3])>>2,pBuff[0]));
	printf(" H Sync Pulse W %d", MAKEWORD(HI4BIT(pBuff[3])&3,pBuff[1]));
	printf(" V Sync Offset %d", MAKEBYTE(LO4BIT(pBuff[3])>>2,HI4BIT(pBuff[2])));
	printf(" V Sync Pulse W %d", MAKEBYTE(LO4BIT(pBuff[3])&3,LO4BIT(pBuff[2])));
#endif
}
...
{
#if __MDINHTX_DBGPRT__ == 1
	printf(" H Image Size %d", MAKEWORD(HI4BIT(pBuff[2]),pBuff[0]));
	printf(" V Image Size %d", MAKEWORD(LO4BIT(pBuff[2]),pBuff[1]));
	printf(" H Border %d V Border %d\n", pBuff[3], pBuff[4]);

	if (pBuff[5]&0x80) printf(" Interlaced");
	else			   printf(" Non-interlaced");

	if (pBuff[5]&0x60) printf(" Table 3.17 for defenition:");
	else			   printf(" Normal Display:");

	if (pBuff[5]&0x10) printf(" Digital\n");
	else			   printf(" Analog\n");
#endif
}
...
static void MDINHTX_ShowParseShortTDPart(PBYTE pBuff, BYTE size)
{
	...
	printf(" CEA861 Modes:\n");
	for (i=0; i<size; i++) {
		printf(" ID %d",(pBuff[i]&0x7f));
		if (pBuff[i]&0x80) printf(" Native mode\n");
		else			   printf(" Non-Native\n");
	}
#endif
}
...
static void MDINHTX_ShowParseShortADPart(PBYTE pBuff)
{
	...
	printf("Audio Format Code %d ", frmt);

	switch (frmt) {
		case  1: printf("Liniar PCM\n"); break;
		case  2: printf("AC-3\n"); break;
		case  3: printf("MPEG-1\n"); break;
		case  4: printf("MP3\n"); break;
		case  5: printf("MPEG2\n"); break;
		case  6: printf("ACC\n"); break;
		case  7: printf("DTS\n"); break;
	...
	printf("Max N of channels %d\n", ((pBuff[0]&0x03)+1));
	if (pBuff[1]&0x01) printf(" Fs: 32KHz\n");
	if (pBuff[1]&0x02) printf(" Fs: 44KHz\n");
	if (pBuff[1]&0x04) printf(" Fs: 48KHz\n");
	if (pBuff[1]&0x08) printf(" Fs: 88KHz\n");
	if (pBuff[1]&0x10) printf(" Fs: 96KHz\n");
	if (pBuff[1]&0x20) printf(" Fs: 176KHz\n");
	if (pBuff[1]&0x40) printf(" Fs: 192KHz\n");

	if (frmt==1) {
		printf("Supported length: ");
		if (pBuff[2]&0x01) printf(" 16bits\n");
		if (pBuff[2]&0x02) printf(" 20bits\n");
		if (pBuff[2]&0x04) printf(" 24bits\n");
	}
	else printf(" Maximum bit rate %d KHz\n", (pBuff[2]<<3));
#endif
}
...
static MDIN_ERROR_t MDINHTX_ParseSpeakDATABlock(BYTE rAddr)
{
	...
	if (rVal&0x01) printf("Speakers' allocation: FL/FR\n");
	if (rVal&0x02) printf("Speakers' allocation: LFE\n");
	if (rVal&0x04) printf("Speakers' allocation: FC\n");
	if (rVal&0x08) printf("Speakers' allocation: RL/RR\n");
	if (rVal&0x10) printf("Speakers' allocation: RC\n");
	if (rVal&0x20) printf("Speakers' allocation: FLC/FRC\n");
	if (rVal&0x40) printf("Speakers' allocation: RLC/RRC\n");
#endif
	return MDIN_NO_ERROR;
}
...
{
#if __MDINHTX_DBGPRT__ == 1
	if (pBuff[0]&0x80) printf(" 720 x  400 @ 70Hz\n");
	if (pBuff[0]&0x40) printf(" 720 x  400 @ 88Hz\n");
	if (pBuff[0]&0x20) printf(" 640 x  480 @ 60Hz\n");
	if (pBuff[0]&0x10) printf(" 640 x  480 @ 67Hz\n");
	if (pBuff[0]&0x08) printf(" 640 x  480 @ 72Hz\n");
	if (pBuff[0]&0x04) printf(" 640 x  480 @ 75Hz\n");
	if (pBuff[0]&0x02) printf(" 800 x  600 @ 56Hz\n");
	if (pBuff[0]&0x01) printf(" 800 x  400 @ 60Hz\n");

	if (pBuff[1]&0x80) printf(" 800 x  600 @ 72Hz\n");
	if (pBuff[1]&0x40) printf(" 800 x  600 @ 75Hz\n");
	if (pBuff[1]&0x20) printf(" 832 x  624 @ 75Hz\n");
	if (pBuff[1]&0x10) printf("1024 x  768 @ 87Hz\n");
	if (pBuff[1]&0x08) printf("1024 x  768 @ 60Hz\n");
	if (pBuff[1]&0x04) printf("1024 x  768 @ 70Hz\n");
	if (pBuff[1]&0x02) printf("1024 x  768 @ 75Hz\n");
	if (pBuff[1]&0x01) printf("1280 x 1024 @ 75Hz\n");

	if (pBuff[2]&0x80) printf("1152 x  870 @ 75Hz\n");

	if ((pBuff[0]==0)&&(pBuff[1]==0)&&(pBuff[2]==0))
		printf("No established video modes\n");
#endif
}
...
static void MDINHTX_ShowParseStandardTiming(PBYTE pBuff, BYTE mode)
{
	...
	for (i=0; mode<TmpVal; mode++) {
		...
		if ((pBuff[i]==0x01)&&(pBuff[i+1]==0x01)) {
			printf(" wasn't defined!\n");
		}
		else {
			printf(" Hor Act pixels %d ", ((pBuff[mode]+31)*8));
			printf(" Aspect ratio: ");
			TmpVal = pBuff[mode+1]&0xc0;
			if(TmpVal==0x00)		printf("16:10");
			else if(TmpVal==0x40)	printf("4:3");
			else if(TmpVal==0x80)	printf("5:4");
			else					printf("16:9");
...
#if __MDINHTX_DBGPRT__ == 1
	printf("[]SendGamutVerticesPacket is called. \n");
#endif
...
#if __MDINHTX_DBGPRT__ == 1
	printf("(Send GamutVerticesPacket)Read INF_CTRL2 %x\n", RegVal);
#endif
	RegVal |= 0x40;
	RegVal |= 0x80;
...
#if __MDINHTX_DBGPRT__ == 1
	RegVal = ReadByteHDMITXP1(0x3f);
	printf("(Send GamutVerticesPacket)Read INF_CTRL2 after set %x\n", RegVal);
#endif
}
}
...
void SendGamutRangePacket(PBYTE data)
{
...
#if __MDINHTX_DBGPRT__ == 1
	printf("[]SendGamutRangePacket is called. \n");
#endif
	...
	RegVal = ReadByteHDMITXP1(0x3f);
#if __MDINHTX_DBGPRT__ == 1
	printf("(Send GamutRangePacket)Read INF_CTRL2 %x\n", RegVal);
#endif
	RegVal |= 0x40;
	RegVal |= 0x80;
	WriteByteHDMITXP1(0x3f, RegVal);
...
{
#if __MDINHTX_DBGPRT__ == 1
	if		(info&0x10)	printf("RI_READING_MORE_ONE_FRAME\n");
	else if (info&0x80)	printf("RI_MISS_MATCH_LAST_FRAME\n");
	else if (info&0x40)	printf("RI_MISS_MATCH_FIRST_FRAME\n");
	else if (info&0x20)	printf("MASK_RI_NOT_CHANGED\n");

	printf("Ri interrupt asserted %d times\n", ++GetFAIL);
#endif
}
...
static void MDINHTX_ShowInfoAuthState(PMDIN_HDMICTRL_INFO pCTL)
{
	...
	printf("{HDMI-Auth}: ");
	switch (pCTL->auth) {
		case HDCP_NOT_DEVICE:	printf("Sink is not HDCP\n"); break;
		case HDCP_BKSV_ERROR:	printf("BKSV Error\n"); break;
		case HDCP_R0s_NOSAME:	printf("R0 mis-match\n"); break;
		case HDCP_AUTHEN_END:	printf("Authenticated\n"); break;
		case HDCP_REPEAT_REQ:	printf("RPT-Auth. Req\n"); break;
		case HDCP_SHACAL_REQ:	printf("SHA calc. Req\n"); break;
		case HDCP_VHx_NOSAME:	printf("VHx Error\n"); break;
		case HDCP_CAS_EXCEED:	printf("MAX_CASCADE_EXCEEDED\n"); break;
		case HDCP_MAX_EXCEED:	printf("MAX_DEVS_EXCEEDED\n"); break;
		case HDCP_DEV_EXCEED:	printf("NUM_DEVS_EXCEEDED\n"); break;
		case HDCP_REAUTH_REQ:	printf("RE-Authen Req\n"); break;
		case HDCP_AUTHEN_REQ:	printf("Authentication Req\n"); break;
		default:				printf("Unknown %02d\n", pCTL->auth); break;
	}
	OldAUTH = pCTL->auth;
#endif
}
...
#if __MDINHTX_DBGPRT__ == 1
	if (count==0) printf("MDINHTX_GetFIFOCntDone %d BYTEs failed\n", bytes);
#endif
...
#if __MDINHTX_DBGPRT__ == 1
	if (GetRPT)	printf("It is Repeater!!\n");
	else		printf("It is TV!!\n");
#endif
...
#if __MDINHTX_DBGPRT__ == 1
	printf("R0RX = 0x%04X, R0TX = 0x%04X\n", R0RX, R0TX);
#endif
...
static MDIN_ERROR_t MDINHTX_SetHDCPAuthMode(PMDIN_HDMICTRL_INFO pCTL)
{
	...
	if (MDINHTX_WriteHDCPTX(0x010, (PBYTE)wBuff, 4)) return MDIN_I2C_ERROR;	  //API v0.31(2012.05.02)
...
#if __MDINHTX_DBGPRT__ == 1
		printf("[HDCP.C](ReAthentication): Call InitDVITX() #1\n");
#endif
	}
...
#if __MDINHTX_DBGPRT__ == 1
		printf("[HDCP.C](ReAthentication): Call InitDVITX() #2\n");
#endif
	}
...
#if __MDINHTX_DBGPRT__ == 1
	printf("Authentication is failure. ReAuthentication.\n");
#endif
...
#if __MDINHTX_DBGPRT__ == 1
	if (count==0) printf("MDINHTX_GetSHAProcDone() TimeOut Error!!!\n");
#endif
...
static MDIN_ERROR_t MDINHTX_HDCPSHAHandler(PMDIN_VIDEO_INFO pINFO)
{
	...
	if (MDINHTX_WriteHDCPTX(0x0ce, (PBYTE)wBuff, 2)) return MDIN_I2C_ERROR;	   //API v0.31(2012.05.02)
...
//	PMDIN_HDMICTRL_INFO pCTL = (PMDIN_HDMICTRL_INFO)&pINFO->stCTL_h;
	...
	else if (pINFO->stOUT_m.mode==MDIN_OUT_MUX656_8) {		// added on 28Dec2011
		switch (pINFO->stVID_h.mode) {
