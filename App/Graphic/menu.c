...
{   
	...
	//------------------------------------------------
	// 메뉴 백그라운드를 검은색 박스로 나타낸다.
	//------------------------------------------------
	//MDINOSD_SetBGBoxColor(RGB(128,16,128));		// set BG-BOX color //검은색
	MDINOSD_SetBGBoxColor(RGB(0,0,0));		// set BG-BOX color //검은색
...
void tPAGE0_KEY(void)
{
 	...
 	switch(key_data)
	{
		...
		case EXIT_KEY :	
			...
			MDINOSD_SetBGBoxColor(RGB(255,255,255));	// set BG-BOX color //흰색
			Set_border_line();
			bMode_change_flag = 1;
			break;
	}
