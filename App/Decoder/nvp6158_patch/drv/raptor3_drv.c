...
#elif defined I2C_INTERNAL 

void __I2CWriteByte8(unsigned char devaddress, unsigned char address, unsigned char data)
{
    int ret;
    unsigned char buf[2];
    struct i2c_client* client = raptor3_client;

    raptor3_client->addr = devaddress;
...
long raptor3_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case IOC_VDEC_AHD_TVI_DISTINGUISH :
			...
			break;
		case IOC_VDEC_CVI_TVI_DISTINGUISH :
			if(copy_from_user( &dev_ch_info, argp, sizeof(decoder_dev_ch_info_s)))
				printk("IOC_VDEC_CVI_TVI_DISTINGUISH error\n");
			video_input_cvi_tvi_distinguish(&dev_ch_info);
			cpy2usr_ret = copy_to_user(argp, &dev_ch_info, sizeof(decoder_dev_ch_info_s));
			break;
        case IOC_VDEC_AHD_CVI_DISTINGUISH :
            if(copy_from_user( &dev_ch_info, argp, sizeof(decoder_dev_ch_info_s)))
                printk("IOC_VDEC_AHD_CVI_DISTINGUISH error\n");
            video_input_ahd_cvi_distinguish(&dev_ch_info);
            cpy2usr_ret = copy_to_user(argp, &dev_ch_info, sizeof(decoder_dev_ch_info_s));
            break;
